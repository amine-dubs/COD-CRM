...
\midrule
\textbf{Ensemble} & \textbf{0.6942} & \textbf{0.9585} & \textbf{0.9789} & \textbf{0.9783} & \textbf{0.9786} \\
\bottomrule
\end{tabularx}
\end{table}
...
\hline
\multicolumn{1}{|l|}{\textbf{R\'{e}el : \'{E}chou\'{e}e}} & 186 (VN) & 407 (FP) \\
\hline
\multicolumn{1}{|l|}{\textbf{R\'{e}el : Livr\'{e}e}} & 418 (FN) & 18~878 (VP) \\
\hline
\end{tabular}
\end{table}
...
L'ensemble atteint un F1-Score de \textbf{0.979} et une AUC-ROC de \textbf{0.694}. L'\'{e}cart entre ces deux m\'{e}triques s'explique par :

\begin{enumerate}
  \item \textbf{Le d\'{e}s\'{e}quilibre des classes} : Avec 97\% de commandes livr\'{e}es, un mod\`{e}le trivial atteignant 97\% d'accuracy. Le F1-Score \'{e}lev\'{e} refl\`{e}te cette facilit\'{e} de pr\'{e}dire la classe majoritaire.
  \item \textbf{L'AUC-ROC mod\'{e}r\'{e}} : L'AUC de 0.694 indique que le mod\`{e}le a une capacit\'{e} limit\'{e}e \`{a} discriminer les \'{e}checs (3\% des commandes). Les features disponibles (temporelles, valeur, produit) ne capturent qu'une partie des facteurs d'\'{e}chec (adresse incorrecte, absence du client, refus).
  \item \textbf{Limitations du dataset Olist} : Les donn\'{e}es br\'{e}siliennes transform\'{e}es ne capturent pas les sp\'{e}cificit\'{e}s du COD alg\'{e}rien. Avec des donn\'{e}es r\'{e}elles (taux d'\'{e}chec de 30-50\%), l'AUC devrait \^{e}tre significativement plus \'{e}lev\'{e} gr\^{a}ce \`{a} des signaux pr\'{e}dictifs plus forts.
\end{enumerate}
...
\midrule
Risque livraison & Classification & CatBoost + LightGBM + XGBoost & AUC-ROC = 0.694, F1 = 0.979 \\
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Pr\'{e}diction du risque par un ensemble de 3 mod\`{e}les (CatBoost, LightGBM, XGBoost) atteignant un F1-Score de 0.979 et un AUC-ROC de 0.694 (m\'{e}triques honn\^{e}tes, sans fuite de donn\'{e}es)
