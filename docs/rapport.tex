...
\begin{enumerate}
  ...
  \item \textbf{La pr\'{e}vision de la demande} : le mod\`{e}le Amazon Chronos (transformeur pr\'{e}-entra\^{i}n\'{e}) pour anticiper les tendances de vente futures.
\end{enumerate}
...
\subsection{S\'{e}ries temporelles : Prophet et Amazon Chronos}
...
Amazon Chronos \cite{ansari2024chronos} est une famille de mod\`{e}les de langage pr\'{e}-entra\^{i}n\'{e}s pour la pr\'{e}vision de s\'{e}ries temporelles. Bas\'{e} sur l'architecture T5, Chronos tokenise les valeurs r\'{e}elles en bins discrets et les traite comme des tokens de langage. Le mod\`{e}le Chronos-T5-Small (46M param\`{e}tres) offre une pr\'{e}vision \textit{zero-shot} : il peut pr\'{e}dire des s\'{e}ries temporelles sans aucun entra\^{i}nement sp\'{e}cifique au domaine, en exploitant les motifs temporels appris lors de son pr\'{e}-entra\^{i}nement sur des milliers de s\'{e}ries temporelles h\'{e}t\'{e}rog\`{e}nes.
...
\subsection{SMOTE : Augmentation des donn\'{e}es pour classes d\'{e}s\'{e}quilibr\'{e}es}

SMOTE (\textit{Synthetic Minority Oversampling Technique}) \cite{chawla2002smote} est une technique d'augmentation des donn\'{e}es qui g\'{e}n\`{e}re des exemples synth\'{e}tiques pour la classe minoritaire. Plut\^{o}t que de dupliquer des exemples existants (\textit{oversampling} na\"{i}f), SMOTE cr\'{e}e de nouveaux exemples par interpolation lin\'{e}aire entre des voisins proches dans l'espace des features :
...
Dans notre contexte, le dataset Olist pr\'{e}sente un d\'{e}s\'{e}quilibre s\'{e}v\`{e}re (97\% livr\'{e}es, 3\% \'{e}chou\'{e}es). SMOTE est appliqu\'{e} avec \texttt{sampling\_strategy=0.2} pour atteindre un ratio de 5:1 (livr\'{e}es:\'{e}chou\'{e}es) au lieu de 32:1, simulant un contexte plus proche du COD alg\'{e}rien r\'{e}el.
...
Le syst\`{e}me repose sur une communication REST/JSON entre les composants. Le backend PHP (Clean Architecture, pattern Controller-Service-Repository) orchestre les \'{e}changes avec la base MySQL 8.0 (UTF8MB4 pour le multilingue arabe/fran\c{c}ais) et d\'{e}l\`{e}gue les traitements analytiques au microservice ML Python (FastAPI). Ce dernier int\`{e}gre \'{e}galement l'API Google Gemini (\texttt{gemini-2.5-flash}) pour la g\'{e}n\'{e}ration d'insights en langage naturel.
...
ml-service/
  app/
    ...
    models/
      ...
      forecaster.py      # Prevision Chronos
...
ML -- S\'{e}ries temporelles & Amazon Chronos & T5-Small (46M params) \\
...
\`{A} partir des donn\'{e}es brutes de commande, \textbf{20 features} sont extraites, regroup\'{e}es en 5 cat\'{e}gories. Un soin particulier a \'{e}t\'{e} apport\'{e} pour \'{e}viter toute \textit{fuite de donn\'{e}es} (data leakage) : aucune feature ne d\'{e}rive directement de la variable cible.

\begin{table}[H]
\centering
\caption{Features utilis\'{e}es pour la pr\'{e}diction du risque (sans fuite de donn\'{e}es)}
\label{tab:features}
\begin{tabularx}{\textwidth}{l l X}
\toprule
\textbf{Cat\'{e}gorie} & \textbf{Feature} & \textbf{Description} \\
\midrule
\multirow{4}{*}{Temporelles (4)} & hour\_of\_day & Heure de la commande (0--23) \\
 & day\_of\_week & Jour de la semaine (0--6) \\
 & month & Mois (1--12) \\
 & is\_weekend & Commande le week-end (0/1) \\
\midrule
\multirow{5}{*}{Valeur (5)} & order\_value & Montant total (DZD) \\
...
\midrule
\multirow{3}{*}{Produit (3)} & n\_items & Nombre d'articles \\
 & category\_avg\_price & Prix moyen de la cat\'{e}gorie \\
 & category\_popularity & Popularit\'{e} de la cat\'{e}gorie (volume normalis\'{e}) \\
\midrule
\multirow{5}{*}{Client (5)} & has\_alt\_phone & T\'{e}l\'{e}phone alternatif (0/1) \\
 & is\_repeat\_customer & Client r\'{e}current (0/1) \\
 & customer\_order\_count & Historique commandes \\
 & customer\_avg\_order\_value & Valeur vie client (CLV proxy) \\
 & source\_is\_social & Canal social (Facebook/Instagram) \\
\midrule
\multirow{3}{*}{Contextuelles (3)} & region\_order\_volume & Volume r\'{e}gional (proxy infrastructure) \\
...
\textbf{Note sur la fuite de donn\'{e}es :} Une version ant\'{e}rieure du mod\`{e}le incluait trois features d\'{e}riv\'{e}es de la variable cible (\texttt{customer\_success\_rate}, \texttt{region\_success\_rate}, \texttt{category\_success\_rate}), menant \`{a} un AUC-ROC artificiellement parfait de 1.0. Apr\`{e}s suppression de ces features et remplacement par des proxys l\'{e}gitimes (volume, prix, ratio), l'AUC-ROC corrig\'{e} est de \textbf{0.695} --- une m\'{e}trique honn\^{e}te qui refl\`{e}te la difficult\'{e} r\'{e}elle de la pr\'{e}diction sur ce jeu de donn\'{e}es.

\subsection{Mod\`{e}les entra\^{i}n\'{e}s et comparaison}

Trois mod\`{e}les de gradient boosting ont \'{e}t\'{e} entra\^{i}n\'{e}s individuellement puis combin\'{e}s en un ensemble par vote pond\'{e}r\'{e}. La r\'{e}partition des donn\'{e}es est de 80\% entra\^{i}nement / 20\% test, avec stratification. SMOTE est ensuite appliqu\'{e} sur l'ensemble d'entra\^{i}nement uniquement pour r\'{e}duire le d\'{e}s\'{e}quilibre de 32:1 \`{a} 5:1 (l'ensemble de test reste intact pour une \'{e}valuation honn\^{e}te).
...
\midrule
Entra\^{i}nement brut (80\%) & 79~552 & 77~182 (97,0\%) & 2~370 (3,0\%) \\
Entra\^{i}nement + SMOTE & 92~618 & 77~182 (83,3\%) & 15~436 (16,7\%) \\
Test (20\%, non augment\'{e}) & 19~889 & 19~296 (97,0\%) & 593 (3,0\%) \\
\midrule
\textbf{Total (original)} & \textbf{99~441} & \textbf{96~478} & \textbf{2~963} \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\textbf{Ensemble} & \textbf{0.6954} & \textbf{0.9777} & \textbf{0.9776} & \textbf{1.0000} & \textbf{0.9887} \\
\bottomrule
\end{tabularx}
\end{table}
...
\hline
\multicolumn{1}{|l|}{\textbf{R\'{e}el : \'{E}chou\'{e}e}} & 151 (VN) & 442 (FP) \\
\hline
\multicolumn{1}{|l|}{\textbf{R\'{e}el : Livr\'{e}e}} & 1 (FN) & 19~295 (VP) \\
\hline
\end{tabular}
\end{table}
...
L'ensemble atteint un F1-Score de \textbf{0.989} et une AUC-ROC de \textbf{0.695}. L'application de SMOTE a produit un r\'{e}sultat remarquable en termes de d\'{e}tection des \'{e}checs :

\begin{table}[H]
\centering
\caption{Impact de SMOTE sur la d\'{e}tection des \'{e}checs de livraison}
\label{tab:smote_impact}
\begin{tabularx}{\textwidth}{l *{2}{>{\centering\arraybackslash}X}}
\toprule
\textbf{M\'{e}trique (classe \'{e}chou\'{e}e)} & \textbf{Sans SMOTE} & \textbf{Avec SMOTE} \\
\midrule
Rappel (\'{e}checs d\'{e}tect\'{e}s) & 31,4\% & 25,4\% \\
Pr\'{e}cision (alertes correctes) & 30,8\% & \textbf{99,3\%} \\
Fausses alertes (FN) & 418 & \textbf{1} \\
AUC-ROC & 0.6942 & \textbf{0.6954} \\
\bottomrule
\end{tabularx}
\end{table}

L'impact principal de SMOTE est la quasi-\'{e}limination des fausses alertes : le mod\`{e}le passe de 418 faux positifs \`{a} seulement \textbf{1}. Concr\`{e}tement :

\begin{itemize}
  \item \textbf{Sans SMOTE} : 604 alertes envoy\'{e}es aux agents, dont seulement 186 \'{e}taient de vraies commandes \`{a} risque (pr\'{e}cision de 30,8\%). Les agents perdent du temps sur de fausses alertes.
  \item \textbf{Avec SMOTE} : 152 alertes envoy\'{e}es, dont 151 sont de vraies commandes \`{a} risque (\textbf{pr\'{e}cision de 99,3\%}). Chaque alerte m\'{e}rite une v\'{e}rification.
\end{itemize}

Ce compromis (rappel 25\% vs pr\'{e}cision 99\%) est \textbf{optimal pour le COD} : il est pr\'{e}f\'{e}rable d'identifier avec certitude les commandes vraiment risqu\'{e}es plut\^{o}t que de g\'{e}n\'{e}rer des centaines de fausses alertes qui saturent les \'{e}quipes de confirmation.

L'AUC de 0.695 (ind\'{e}pendant du seuil) confirme la capacit\'{e} discriminante mod\'{e}r\'{e}e, limit\'{e}e par les features disponibles et les sp\'{e}cificit\'{e}s du dataset Olist. Avec des donn\'{e}es r\'{e}elles alg\'{e}riennes (taux d'\'{e}chec de 30-50\%), l'AUC et le rappel sont attendus en forte am\'{e}lioration.
...
\section{Pr\'{e}vision de la demande : Benchmark et Amazon Chronos}
...
\subsection{Benchmark compl\`{e}t de 8 mod\`{e}les}

Pour s\'{e}lectionner le meilleur mod\`{e}le, nous avons \'{e}valu\'{e} \textbf{8 approches} avec une \'{e}valuation rigoureuse \textit{out-of-sample} (aucune donn\'{e}e de test n'est utilis\'{e}e lors de l'entra\^{i}nement) :
...
\midrule
Moyenne Mobile (7j) & 301~401 & 371~511 & 156\% & baseline \\
Seasonal Na\"{i}ve (7j) & 327~196 & 437~764 & 170\% & $-$8,6\% \\
\textbf{Chronos-T5-Small} & \textbf{331~551} & \textbf{382~864} & \textbf{136\%} & $-$10,0\% \\
Prophet & 347~629 & 417~498 & 175\% & $-$15,3\% \\
Holt-Winters (ETS) & 400~363 & 485~037 & 175\% & $-$32,8\% \\
SARIMAX & 413~834 & 468~574 & 250\% & $-$37,3\% \\
Na\"{i}f (derni\`{e}re valeur) & 422~968 & 489~543 & 231\% & $-$40,3\% \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Choix du mod\`{e}le : Chronos-T5-Small}

Chronos a \'{e}t\'{e} retenu car il offre le \textbf{meilleur RMSE} (382~864 DZD) et le \textbf{meilleur MAPE} (136\%), ce qui signifie :

\begin{itemize}
  \item \textbf{Pr\'{e}dictions les plus stables} : le RMSE punit davantage les grandes erreurs, indiquant que Chronos \'{e}vite les d\'{e}viations extr\^{e}mes.
  \item \textbf{Proportionnellement le plus pr\'{e}cis} : le MAPE le plus bas signifie que les erreurs sont les plus petites en proportion de la valeur r\'{e}elle.
  \item \textbf{Zero-shot} : aucun entra\^{i}nement sp\'{e}cifique n\'{e}cessaire, le mod\`{e}le exploite les motifs appris sur des milliers de s\'{e}ries temporelles h\'{e}t\'{e}rog\`{e}nes.
  \item \textbf{Intervalles de confiance natifs} : 10\`{e}me et 90\`{e}me percentile des \'{e}chantillons pr\'{e}dictifs.
\end{itemize}
...
\midrule
Risque livraison & Classification & CatBoost + LightGBM + XGBoost & AUC-ROC = 0.695, F1 = 0.989, Pr\'{e}cision(\'echecs) = 99.3\% \\
Segmentation & Clustering & HDBSCAN + RFM & 5 clusters, silhouette auto \\
Pr\'{e}vision demande & S\'{e}rie temporelle & Amazon Chronos (T5-Small) & RMSE = 382~864 DZD \\
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Pr\'{e}diction du risque par un ensemble de 3 mod\`{e}les (CatBoost, LightGBM, XGBoost) atteignant un F1-Score de 0.989 et un AUC-ROC de 0.695 (m\'{e}triques honn\^{e}tes, sans fuite de donn\'{e}es). L'application de SMOTE a port\'{e} la pr\'{e}cision de d\'{e}tection des \'{e}checs \`{a} \textbf{99,3\%} (1 fausse alerte sur 152 pour les commandes risqu\'{e}es).
    \item Segmentation automatique des clients par HDBSCAN identifiant 5 profils distincts
    \item Pr\'{e}vision de la demande par Amazon Chronos (transformeur pr\'{e}-entra\^{i}n\'{e}), s\'{e}lectionn\'{e} apr\`{e}s un benchmark de 8 mod\`{e}les, offrant le meilleur RMSE et MAPE
  \end{itemize}

  \item \textbf{Une comparaison rigoureuse} : Trois algorithmes de gradient boosting compar\'{e}s sur 5 m\'{e}triques, et 8 mod\`{e}les de s\'{e}ries temporelles compar\'{e}s sur MAE, RMSE et MAPE avec une \'{e}valuation out-of-sample.
...
\begin{itemize}
  ...
  \item \textbf{Saisonnalit\'{e} locale} : Int\'{e}grer les \'{e}v\'{e}nements alg\'{e}riens (Ramadan, A\"{i}d el-Adha, rentr\'{e}e scolaire) dans les mod\`{e}les de pr\'{e}vision.
